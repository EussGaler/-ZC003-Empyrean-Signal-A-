 ...
 * @return uint16_t 最终结果（众数或中位数）
 */
uint32_t Result_Calculate2(uint32_t *IndexDiff, uint16_t len)
{
    if (len == 0) return 0; //空数组保护
...
void Calculate_and_Show(void)
{
    ...
    if (ADC_Flag == 0) //MHz级别
    {
        ...
        if (SampleCount < 10)
        {
            FreqTemp[SampleCount] = Freq;
            SampleCount ++;
            delay_ms(200);
            DL_Timer_startCounter(TIMER_ADC_INST);
        }
        else
        {
            Freq = Result_Calculate2(FreqTemp, 10);
        }
    }
    else if (ADC_Flag == 1) //kHz级别
    {
        ...
        if (SampleCount < 10)
        {
            ...
            SampleCount ++;
            delay_ms(200);
            DL_Timer_startCounter(TIMER_ADC2_INST);
        }
        else
        {
            Freq2 = Result_Calculate2(FreqTemp, 10);
        }
    }
